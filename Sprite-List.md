#Sprite List

This is a complete mapping of the sprites used (and unused) in this game, by checking the sprite type table, the address **$7E0946**.<br>Note: the sprite loaders are only found when the level is loading (black screen), and maybe are used during the level in the code internally. They load the sprite properly, you can check this by poking a value during the level, the loader sprite lasts for just one frame, then it loads the respective sprite.

ID|Name|Appearing|Notes
:--:| :------ | :------ | :------ |
00|*(Empty sprite slot)*|General|
01|Characters in normal state; Car parts; Arrows (selection)|Cutscenes; Bedrock; Options and Language|Kids, Barney, Cliff, Wilma, Miss Sharon Stone, Lever, Concrete
02|Characters in action|Cutscenes|Barney scared, Cliff walking, Wilma scared, Kids running
03|Cliff disguised as Gorilla (falling)|Cutscene|
04|Sparkle loader|Title screen|
05|Sparkle|Title screen|
06|Asteroid loader|Title screen|
07|Asteroid|Title screen|
08|Planet loader|Title screen|
09|Planet|Title screen|
0A|Earth loader|Title screen|
0B|Earth|Title screen|
0C|Moving planet or star loader|Title screen|
0D|Moving planet or star|Title screen|
0E|Stone (when sprite) loader|General|
0F|Stone (when sprite)|General|
10|Stone (when extended sprite)|General|First frame being thrown
11|Stone (when extended sprite)|General|
12|Bowling ball (when sprite) loader|General|
13|Bowling ball (when sprite)|General|
14|Bowling ball (when extended sprite)|General|First frame being thrown
15|Bowling ball (when extended sprite)|General|
16|Boulder (platform) loader|General|
17|Boulder (platform)|General|
18|Rock with items loader|General|
19|Rock with items|General|
1A|Smoke spawner on chimney loader|Game Over|
1B|Smoke spawner on chimney|Game Over|
1C|Smoke from chimney loader|Game Over|
1D|Smoke from chimney|Game Over|
1E|Dust from walking loader|General|
1F|Dust from walking|General|
20|Items from rocks loader|General|
21|Items from rocks|General|It can be Trophy, Life, Hourglass, Gem, Sphere, Big diamond, or Big gem, according to the Misc table 1.
22|Gem (spawned naturaly) loader|General|
23|Gem (spawned naturaly)|General|
24|Trophy (spawned naturaly) loader|Jungle|
25|Trophy (spawned naturaly)|Junlge|
26|Spikeball loader|Volcanic|
27|Spikeball|Volcanic|
28|Life loader|Quarry 3|
29|Life|Quarry 3|
2A|Button on ground loader|Quarry, Machine|Activates sprite 33 and controls sprite 7C
2B|Button on ground|Quarry, Machine|Activates sprite 33 and controls sprite 7C
2C|**_[unused]_** Button on ground loader|---------------|
2D|**_[unused]_** Button on ground|---------------|
2E|**_[unused]_** Coin thing loader|---------------|
2F|**_[unused]_** Coin thing|---------------|Rotates a bit when Fred pass through it for the first time
30|Disappearing platform loader|Volcanic|
31|Disappearing platform|Volcanic|
32|Hidden platform loader|Quarry, Machine|
33|Hidden platform|Quarry, Machine|Activated by sprites 2B or A8
34|Floating rock platform loader|Volcanic|
35|Floating rock platform |Volcanic|
36|Floating rock platform (moving) loader|Volcanic|
37|Floating rock platform (moving)|Volcanic|
38|Floating rock platform (moving) loader|Machine|
39|Floating rock platform (moving)|Machine|
3A|**_[unused]_** Floating rock platform loader|---------------|
3B|**_[unused]_** Floating rock platform|---------------|Sometimes you can't jump when standing on it
3C|**_[unused]_** Floating rock platform loader|---------------|
3D|**_[unused]_** Floating rock platform|---------------|Sometimes you can't jump when standing on it
3E|Lava ball loader|Volcanic|
3F|Lava ball|Volcanic|
40|Lava splash (first frame)|Volcanic|
41|Lava splash from Lava ball or Spikeball|Volcanic|
42|Wall lava hole loader|Volcanic|
43|Wall lava hole|Volcanic|
44|Jumping lava loader|Volcanic|
45|Jumping lava|Volcanic|
46|Jumping lava spawner loader|Volcanic|
47|Jumping lava spawner|Volcanic|
48|**_[unused]_** ??? loader|---------------|
49|**_[unused]_** ???|---------------|Uses part of Fred's graphics
4A|Flame on wall loader|Volcanic|
4B|Flame on wall|Volcanic|
4C|Fire from steam (lava/duct) loader|Volcanic/Machine|
4D|Fire from steam (lava/duct)|Volcanic/Machine|
4E|Big falling lava boulder loader|Volcanic|
4F|Big falling lava boulder|Volcanic|
50|Falling lava boulder loader|Volcanic|
51|Falling lava boulder|Volcanic|Rolls on ground after
52|Falling lava boulder loader|Volcanic|
53|Falling lava boulder|Volcanic|
54|Giant rolling boulder loader|Machine|
55|Giant rolling boulder|Machine|
56|Bouncing toxic rock loader|Machine|
57|Bouncing toxic rock|Machine|
58|Sprite dying|General|Frog, Worker, Gorilla, Tar creature, Deadly plant, Monkey, Climbing lizard
59|Falling boulder spawner loader|Machine|
5A|Falling boulder spawner|Machine|
5B|Rock and toxic rock from tubes loader|Machine|
5C|Rock and toxic rock from tubes|Machine|
5D|Falling boulder loader|Machine|
5E|Falling boulder|Machine|
5F|Monkey loader|Volcanic|
60|Monkey|Volcanic|
61|**_[unused]_** ??? loader|---------------|
62|**_[unused]_** ???|---------------|Uses part of Fred's graphics
63|Bone from eating idle animation loader|General|
64|Bone from eating idle animation|General|
65|Boulder limit rock loader|General|
66|Boulder limit rock|General|
67|Springboard contraption loader|Quarry, Machine|
68|Springboard contraption|Quarry, Machine|Permanent sprite. Activated by Boar dinos or bowling balls
69|Boar dino loader|Quarry, Machine|
6A|Boar dino|Quarry, Machine|
6B|**_[unused]_** Falling rock loader|---------------|
6C|**_[unused]_** Falling rock|---------------|Same graphics from sprite 19
6D|Boulder that floats on lava loader|Volcanic|
6E|Boulder that floats on lava|Volcanic|
6F|Climbing lizard loader|Volcanic|
70|Climbing lizard|Volcanic|
71|Frog loader|Quarry, Machine|
72|Frog|Quarry, Machine|
73|Mechanical platform loader|Machine|
74|Mechanical platform|Machine|
75|Circular saw loader|Machine|
76|Circular saw|Machine|
77|Switchable track loader|Machine|
78|Switchable track|Machine|
79|Smashing square stone loader|Machine|
7A|Smashing square stone|Machine|
7B|Smashing square stone controlled by Button (2B) loader|Machine|
7C|Smashing square stone controlled by Button (2B)|Machine|Permanent sprite
7D|**_[unused]_** Bouncing rock loader|---------------|
7E|**_[unused]_** Bouncing rock|---------------|Same graphics from sprite 19
7F|**_[unused]_** ???loader|---------------|
80|**_[unused]_** ???|---------------|Uses Walking egg graphics, kinda. Crashes the game.
81|Walking egg loader|Quarry, Machine|
82|Walking egg|Quarry, Machine|
83|Walking egg hatching|Quarry, Machine|
84|Baby pterodactyl from Walking egg|Quarry, Machine|
85|Gorilla loader|Jungle|
86|Gorilla|Jungle|
87|Falling leaves loader|Jungle|
88|Falling leaves|Jungle|From hidden Gorilla
89|Level end bird loader|General|
8A|Level end bird|General|
8B|Stick  from Level end bird loader|General|
8C|Stick  from Level end bird|General|
8D|Platform in mud loader|Jungle|
8E|Platform in mud|Jungle|
8F|Button on wall loader|Machine|
90|Button on wall|Machine|Permanent sprite, changes sprite 78 direction
91|Platform in Lava jet loader|Volcanic|
92|Platform in Lava jet|Volcanic|
93|Lava jet loader|Volcanic|
94|Lava jet|Volcanic|
95|Brown smoke spawner loader|Volcanic|
96|Brown smoke spawner|Volcanic|
97|Brown smoke (first frame)|Volcanic|
98|Brown smoke|Volcanic|
99|Floating rock platform that burns near lava loader|Volcanic|
9A|Floating rock platform that burns near lava|Volcanic|
9B|Big square stone loader|Machine|
9C|Big square stone|Machine|
9D|Worker that throws Spinning discs loader|Quarry, Machine|
9E|Worker that throws Spinning discs|Quarry, Machine|
9F|Worker loader|Quarry, Machine|
A0|Worker|Quarry, Machine|
A1|Worker pushing boulder loader|Quarry, Machine|
A2|Worker pushing boulder|Quarry, Machine|Then becomes A0
A3|**_[unused]_** ??? loader|---------------|
A4|**_[unused]_** ???|---------------|Unfinished Worker that throws, maybe. Crashes the game.
A5|Worker loader|Quarry, Machine|
A6|Worker|Quarry, Machine|
A7|Lizard on contraption loader|Quarry, Machine|
A8|Lizard on contraption|Quarry, Machine|Activates sprite 33
A9|Spinning disc from Worker loader|Quarry, Machine|
AA|Spinning disc from Worker|Quarry, Machine|
AB|Boulder from Worker loader|Quarry, Machine|
AC|Boulder from Worker|Quarry, Machine|
AD|**_[unused]_** ??? loader|---------------|
AE|**_[unused]_** ???|---------------|Lays the sprite 80
AF|Caveman boss loader|Quarry 3|
B0|Caveman boss|Quarry 3|
B1|Caveman boss projectile loader|Quarry 3|
B2|Caveman boss projectile|Quarry 3|
B3|Rock from smashed rock (sprite 19) or boulder (sprite  5E) loader|General|
B4|Rock from smashed rock (sprite 19) or boulder (sprite  5E)|General|
B5|Tiger boss loader|Jungle 4|
B6|Tiger boss|Jungle 4|
B7|Coconut on tree loader|Jungle 4|
B8|Coconut on tree|Jungle 4|
B9|Coconut falling loader|Jungle 4|
BA|Coconut falling|Jungle 4|
BB|Coconut hitting the ground|Jungle 4|
BC|Coconut half|Jungle 4|
BD|Sparkles from hitted Tiger loader|Jungle 4|
BE|Sparkles from hitted Tiger|Jungle 4|
BF|Boulder (rolling by itself) loader|Machine|
C0|Boulder (rolling by itself)|Machine|
C1|Surf board loader|Volcanic|
C2|Surf board|Volcanic|
C3|Sparkle from item loader|General|
C4|Sparkle from item|General|
C5|Bamm-Bamm loader|Bedrock|
C6|Bamm-Bamm and Pebbles first frame or touching things|Bedrock|Things: car, pterodactyls, balconies
C7|Bamm-Bamm and Pebbles jumping from car|Bedrock|
C8|Bamm-Bamm and Pebbles|Bedrock|Permanent sprite
C9|Bamm-Bamm and Pebbles falling on ground|Bedrock|
CA|Pebbles loader|Bedrock|
CB|Pterodactyl standing still loader|Bedrock|
CC|Pterodactyl standing still|Bedrock|
CD|Pterodactyl bounced off|Bedrock|
CE|Pterodactyl flying upwards|Bedrock|
CF|Orange gem|Bedrock|
D0|Orange gem falling|Bedrock|
D1|"Tongue" from Deadly plant (in mouth)|Jungle|
D2|"Tongue" from Deadly plant (growing)|Jungle|
D3|"Tongue" from Deadly plant (full growth)|Jungle|
D4|"Tongue" from Deadly plant (shrinking)|Jungle|
D5|First bridge segment loader|Jungle|
D6|First bridge segment|Jungle|Permanent sprite
D7|First bridge segment jumping|Jungle|
D8|First bridge segment attaching|Jungle|
D9|Bridge segment (not ready) loader|Jungle|
DA|Bridge segment (not ready)|Jungle|
DB|Bridge segment attaching|Jungle|
DC|Bridge segment forming bridge|Jungle|Permanent sprite
DD|Bridge segment falling|Jungle|
DE|Woodpecker on bridge loader|Jungle|
DF|Woodpecker on bridge|Jungle|
E0|Woodpecker pecking|Jungle|
E1|Woodpecker after pecking|Jungle|Permanent sprite
E2|Mud bubbles loader|Jungle|
E3|Mud bubbles|Jungle|
E4|Tar projectile  first frame being thrown|Jungle|
E5|Tar projectile|Jungle|
E6|Vine that falls loader|Jungle|
E7|Vine that falls|Jungle|
E8|Cutting effect from falling vine loader|Jungle|
E9|Cutting effect from falling vine|Jungle|
EA|Mammoth standing still loader|Jungle|
EB|Mammoth standing still|Jungle|
EC|Mammoth running|Jungle|
ED|Bamm-Bamm and Pebbles symbols loader|Bedrock|
EE|Bamm-Bamm and Pebbles symbols|Bedrock|
EF|Arrow pointing kids loader|Bedrock|
F0|Arrow pointing kids|Bedrock|
F1|Deadly plant (standing still) loader|Jungle|
F2|Deadly plant (standing still)|Jungle|
F3|Deadly plant (using "tongue")|Jungle|
F4|Plant that ejects Fred loader|Jungle|
F5|Plant that ejects Fred|Jungle|
F6|Tar creature loader|Jungle|
F7|Tar creature|Jungle|
F8|Boss HP star loader|Bosses|
F9|Boss HP star|Bosses|
FA|**_[unused]_**|---------------|Crashes the game
FB|**_[unused]_**|---------------|Crashes the game
FC|**_[unused]_**|---------------|Crashes the game
FD|**_[unused]_**|---------------|Crashes the game
FE|**_[unused]_**|---------------|Crashes the game
FF|**_[unused]_**|---------------|Crashes the game
